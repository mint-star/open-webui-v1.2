...
<div class="flex lg:hidden items-center">
  <DropdownMenu.Root>
    ...
    <DropdownMenu.Content class="bg-white border border-gray-200 rounded shadow-lg mt-2 py-2 w-[260px] z-[999]">
      {#each apps as suite}
        <div class="px-4 py-2 text-sm font-semibold uppercase flex items-center gap-2" style="background: #{suite.bg}">
        ...
        {#each suite.apps as app}
          <DropdownMenu.Item>
            <a
              ...
              class="px-4 py-2 flex items-center gap-3 text-gray-900 hover:bg-indigo-50 { !app.access ? 'opacity-60' : '' }"
            >
...
<div class="hidden lg:flex items-center gap-2">
  {#each apps as suite}
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Content class="bg-white border border-gray-200 rounded shadow-lg mt-2 py-1 w-[250px] z-[999]">
        {#each suite.apps as app}
          <DropdownMenu.Item>
            <a
              href={app.href}
              on:click={(e) => handleAppClick(e, app)}
              class="px-4 py-3 flex items-center gap-3 text-gray-900 hover:bg-indigo-50 { !app.access ? 'opacity-60' : '' }"
            >
              <img src={app.icon} alt={app.label} class="w-5 h-5 shrink-0" />
              <span class="text-sm font-medium">{app.label}</span>
              {#if !app.access}
...
<Dialog.Root bind:open={showNoAccessModal}>
  <Dialog.Portal>
    ...
    >
      <Dialog.Title class="text-lg font-bold text-red-600 mb-2">
        Access Denied
      </Dialog.Title>

      <Dialog.Description class="text-gray-800 mb-4 text-sm">
        We're sorry, but you do not currently have permission to view this
        information. For assistance or to request access, please contact
