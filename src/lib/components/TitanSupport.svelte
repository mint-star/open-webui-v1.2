<script lang="ts">
  ...
  import { PUBLIC_GRANT_TYPE, PUBLIC_CLIENT_ID, PUBLIC_CLIENT_SECRET, PUBLIC_MYSPACE_API_URL } from '$env/static/public';
  ...
  async function fetchBearerToken(): Promise<void> {
    ...
    try {
      const response = await fetch(
        `${PUBLIC_MYSPACE_API_URL}/oauth/token`,
        {
          method: "POST",
          headers: {
  ...
  async function handlePasteFromClipboard(): Promise<void> {
    ...
    try {
      ...
      if (imageItems.length === 0) {
        fileErrorMessage = "No image found in clipboard. Try Ctrl+V / Cmd+V after copying a screenshot.";
        return;
      }
      ...
      fileErrorMessage = "Couldn't access clipboard. Use Ctrl+V / Cmd+V to paste a screenshot directly.";
...
{#if showSuccessModal}
  <div class="fixed inset-0 z-[200] flex items-center justify-center p-4">
    ...
    <div class="relative z-10 bg-white dark:bg-gray-850 rounded-2xl shadow-2xl p-8 w-full max-w-sm flex flex-col items-center text-center">
      <!-- Animated tick -->
      <div class="w-20 h-20 rounded-full bg-green-50 dark:bg-green-900/20 flex items-center justify-center mb-5">
      ...
      <h2 class="text-xl font-bold text-slate-900 dark:text-gray-100 mb-2">
        Request Submitted!
      </h2>
    ...
    <Dialog.Content class="fixed inset-0 z-101 overflow-y-auto">
      <div class="flex min-h-full items-start justify-center py-10 px-4">
        <div class="w-full max-w-2xl bg-white dark:bg-gray-850 rounded-2xl shadow-xl p-6 relative">
          ...
          <!-- Token loading state -->
          {#if isFetchingToken}
            <div class="flex items-center justify-center gap-2 py-8 text-slate-400 dark:text-gray-500 text-sm">
          ...
          <div class="mb-6">
            ...
            <div class="flex gap-2 flex-wrap">
              ...
              >
                <svg class="w-[14px] h-[14px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
          ...
          <div class="mb-6">
            ...
            <div class="relative">
              ...
              ></textarea>
              <span class="absolute bottom-[7px] left-[1px] right-[1px] text-xs text-slate-400 px-3 py-2 text-right bg-white rounded-b-2xl border-t-1 border-slate-200 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-200">
          ...
          <div class="mb-6">
            <div class="flex items-center justify-between">
              <div class="text-sm font-medium text-slate-800 dark:text-gray-200">
                Screenshots
                <span class="text-slate-400 dark:text-gray-500 font-normal text-sm">(optional)</span>
              </div>
            ...
            <p class="text-slate-500 dark:text-gray-500 text-[13px] mt-0.5 mb-2.5 opacity-80 mb-2.5">Max 20 files · PNG, JPG only</p>
            ...
            >
              <svg class="w-5 h-5 text-slate-400 dark:text-gray-400 shrink-0 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                <polyline points="17 8 12 3 7 8"/>
                <line x1="12" y1="3" x2="12" y2="15"/>
              </svg>
              <p class="text-sm text-slate-400 dark:text-gray-400 pointer-events-none select-none">
                {#if uploadedFiles.length >= maxFiles}
                  Maximum of {maxFiles} screenshots reached
                {:else if isDragging}
                  Release to upload
                {:else}
                  Drag & drop, <span class="font-semibold text-slate-700 dark:text-gray-200">click to upload</span>, or paste with Ctrl/Cmd + V
                {/if}
              </p>
            </div>
              ...
              <div class="flex flex-wrap gap-3">
                {#each uploadedFiles as file (file.id)}
                  <div class="relative w-[80px] h-[80px] group">
                    ...
                    <div class="absolute -bottom-8 left-1/2 -translate-x-1/2 bg-black/75 text-white text-xs px-2 py-1 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                      {file.name}
                    </div>
                  </div>
                {/each}
